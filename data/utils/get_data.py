...
def get_distinct_df(tablename, cols=None):
    conn = imh.connect_to_heroku_db()
    order = ' order by ain, landbaseyear desc'
    if cols is None:
        df = pd.read_sql_query(f'select distinct on (ain) * from {tablename} {order}', con=conn)
    else:
        cols_as_string = ', '.join(cols)
        df = pd.read_sql_query(f'select distinct on (ain) {cols_as_string} from {tablename} {order}', con=conn)
...
def get_past4y_df(tablename, cols=None):
    ...
    order = ' order by ain, landbaseyear desc'
    if cols is None:
        df = pd.read_sql_query(f'select distinct on (ain) * from {tablename} {year_clause} {order}', con=conn)
    else:
        cols_as_string = ', '.join(cols)
        df = pd.read_sql_query(f'select distinct on (ain) {cols_as_string} from {tablename} {year_clause} {order}', con=conn)
...
def get_2021_df(tablename, cols=None):
    conn = imh.connect_to_heroku_db()
    year_clause = ' where landbaseyear = \'2021\''
    if cols is None:
        df = pd.read_sql_query(f'select * from {tablename} {year_clause}', con=conn)
    else:
        cols_as_string = ', '.join(cols)
        df = pd.read_sql_query(f'select distinct on (ain) {cols_as_string} from {tablename} {year_clause}', con=conn)
    return df
